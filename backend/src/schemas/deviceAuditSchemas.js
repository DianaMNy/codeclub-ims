...
const { z } = require('zod');
const { shortText, longText } = require('./common');
...
const deviceAuditSchema = z.object({
  ...
  total_devices: z.coerce.number().int().nonnegative().optional().nullable(),
  functioning_devices: z.coerce.number().int().nonnegative().optional().nullable(),
  faulty_devices: z.coerce.number().int().nonnegative().optional().nullable(),
