...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    if (role === 'admin' || role === 'programme_coordinator') {
      query = `SELECT so.*, sc.official_name AS school_name, sc.club_id, sc.county, sc.type AS school_type, m.full_name AS mentor_name, p.name AS pathway_name, p.icon AS pathway_icon FROM session_observations so LEFT JOIN schools_and_centres sc ON so.school_id = sc.id LEFT JOIN mentors m ON so.mentor_id = m.id LEFT JOIN pathways p ON so.pathway_id = p.id ORDER BY so.date_of_visit DESC`;
      params = [];
    } else {
...
router.get('/school/:schoolId', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(`SELECT so.*, m.full_name AS mentor_name, p.name AS pathway_name, p.icon AS pathway_icon FROM session_observations so LEFT JOIN mentors m ON so.mentor_id = m.id LEFT JOIN pathways p ON so.pathway_id = p.id WHERE so.school_id = $1 ORDER BY so.visit_number ASC`, [req.params.schoolId]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/pathways-with-projects', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(`SELECT id, name, key, icon, color, levels, projects FROM pathways ORDER BY name`);
