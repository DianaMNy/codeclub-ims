...
router.post('/', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { full_name, phone, email, school_id, training_completed, safeguarding_done, survey_done, role, county } = req.body;
    const result = await pool.query(`
      INSERT INTO heads_of_school (full_name, phone, email, school_id, training_completed, safeguarding_done, survey_done, role, county)
  ...
  try {
    ...
    const result = await pool.query(`
      UPDATE heads_of_school SET
        ...
        training_completed=$5, safeguarding_done=$6, survey_done=$7,
        role=$8, county=$9
